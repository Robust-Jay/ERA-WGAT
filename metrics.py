...
class Torch_SSIM(object):
    '''
    modified from https://github.com/jorge-pessoa/pytorch-msssim
    '''
    def __init__(self, des="structural similarity index", data_range=1):
        self.des = des
        self.L = data_range
...
class VGGP(object):
    def __init__(self):
        ...
        self.__feature_extractor = nn.Sequential(*list(vgg.features.children())[:35])  # noqa

    def __call__(self, y_pred, y_true):
        self.__feature_extractor.to(y_pred.device)
        if y_pred.shape[1] != 3:
            y_pred = y_pred.repeat(1, 3, 1, 1)
            y_true = y_true.repeat(1, 3, 1, 1)
        y_pred = self.__feature_extractor(y_pred)
        y_true = self.__feature_extractor(y_true)
        return torch.mean((y_pred - y_true) ** 2)
